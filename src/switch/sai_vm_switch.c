...
static void sai_vm_free_table_resources (void)
{
    ...
    for (table_id = 0; table_id < SAI_ACL_TABLE_ID_MAX; table_id++) {
        if (g_sai_vm_tcb.acl_entry_id_bitmap [table_id]) {
            free (g_sai_vm_tcb.acl_entry_id_bitmap [table_id]);
        }
...
static sai_status_t sai_vm_tables_init (void)
{
    ...
    do {
        ...
        for (table_id = 0; table_id < SAI_ACL_TABLE_ID_MAX; table_id++) {
            g_sai_vm_tcb.acl_entry_id_bitmap [table_id] =
                std_bitmap_create_array (SAI_VM_ACL_TABLE_MAX_ENTRIES);

            if (!(g_sai_vm_tcb.acl_entry_id_bitmap [table_id])) {
                SAI_SWITCH_LOG_ERR ("Failed to create ACL entry ID bitmap "
                                    "array of size %d for TABLE ID %d.",
                                    SAI_VM_ACL_TABLE_MAX_ENTRIES, table_id);
