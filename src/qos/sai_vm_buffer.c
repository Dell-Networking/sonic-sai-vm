                                                ...
                                                sai_object_id_t *p_buf_pool_oid)
{
    ...
    if(p_buf_pool_node->pool_type == SAI_BUFFER_POOL_INGRESS) {
    ...
    } else if (p_buf_pool_node->pool_type == SAI_BUFFER_POOL_EGRESS) {
...
static sai_status_t sai_vm_pg_stats_get (sai_object_id_t pg_id,
                                         const sai_ingress_priority_group_stat_counter_t
                                         *counter_ids, uint32_t number_of_counters,
                                         uint64_t* counters)
{
    STD_ASSERT (counter_ids != NULL);
