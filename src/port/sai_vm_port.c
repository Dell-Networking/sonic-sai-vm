                                                         ...
                                                         const sai_attribute_t *attr)
{
    ...
    sai_attr_set.id = SAI_PORT_ATTR_OPER_STATUS;
    sai_attr_set.value.s32 = (( (attr->value.s32 == SAI_PORT_INTERNAL_LOOPBACK_PHY) ||
    ...
    for(attr_idx = 0; attr_idx < attr_count; attr_idx++) {
        ...
        switch(attr_list[attr_idx].id) {
            ...
            case SAI_PORT_ATTR_SUPPORTED_SPEED:
                ret_code = sai_port_attr_supported_speed_get(port_id,
                                                             &attr_list[attr_idx].value);
                break;
        ...
        if (ret_code != SAI_STATUS_SUCCESS) {
            SAI_PORT_LOG_ERR ("Attr get for port id 0x%"PRIx64"'s attr 0x%x index %d "
                              "failed with err %d", port_id, attr_list[attr_idx].id,
                              attr_idx, ret_code);
...
static sai_status_t sai_npu_port_get_stats (sai_object_id_t port_id,
                                            const sai_port_stat_counter_t *counter_ids,
