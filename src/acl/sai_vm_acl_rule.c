                                             ...
                                             bool isCreate)
{
    ...
    uint_t rule_filter_to_tbl_field_map [] = {
        ...
        SAI_ACL_TABLE_ATTR_FIELD_TCP_FLAGS, SAI_ACL_TABLE_ATTR_FIELD_IP_TYPE,
                                      ...
                                      sai_acl_rule_t *acl_rule)
{
    ...
    sai_object_id_t     entry_obj_id = 0;
    uint_t              entry_id = 0;
    int                 free_idx = 0;
    ...
    free_idx = std_find_first_bit (sai_vm_access_acl_entry_bitmap (table_id),
                                   SAI_VM_ACL_TABLE_MAX_ENTRIES, 0);

    if (free_idx < 0) {
        SAI_ACL_LOG_ERR ("No more free entries available on ACL TABLE %d "
    ...
    entry_id = sai_vm_acl_bmp_idx_to_entry_id_get (table_id, free_idx);

    entry_obj_id = sai_uoid_create (SAI_OBJECT_TYPE_ACL_ENTRY,
                                    (sai_npu_object_id_t) entry_id);

    acl_rule->rule_key.acl_id = entry_obj_id;
    ...
    if (sai_rc != SAI_STATUS_SUCCESS) {
        SAI_ACL_LOG_ERR ("Error inserting entry to DB for ACL Entry: %d, "
                         "OBJ ID: %"PRIx64".", entry_id, entry_obj_id);

        return sai_rc;
    }

    STD_BIT_ARRAY_CLR (sai_vm_access_acl_entry_bitmap (table_id), free_idx);

    SAI_ACL_LOG_TRACE ("ACL Entry Creation success, Entry ID: %d, Obj Id: "
                       " 0x%"PRIx64" on Table Id: %d, Obj Id: 0x%"PRIx64".",
                       entry_id, entry_obj_id, table_id,
                          acl_table->table_key.acl_table_id);
                                      ...
                                      sai_acl_rule_t *acl_rule)
{
    ...
    bitmap_idx = sai_vm_acl_entry_id_to_bmp_idx_get (entry_id);

    STD_BIT_ARRAY_SET (sai_vm_access_acl_entry_bitmap (table_id), bitmap_idx);
