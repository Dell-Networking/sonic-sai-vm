...
sai_status_t sai_npu_lag_ports_set (sai_object_id_t lag_id,
                                    const sai_object_list_t *lag_port_list)
{
    SAI_LAG_LOG_TRACE ("LAG Add set, ID: 0x%"PRIx64".", lag_id);

    STD_ASSERT (lag_port_list != NULL);

    return SAI_STATUS_SUCCESS;
}
static sai_status_t sai_npu_lag_port_mode_set (sai_object_id_t lag_id,
                                               sai_object_id_t port_id, sai_lag_port_mode_t mode)
{
    SAI_LAG_LOG_TRACE ("LAG port mode set, LAG ID: 0x%"PRIx64" Port ID: 0x%"PRIx64".",
                       lag_id, port_id);

    return SAI_STATUS_SUCCESS;

}
static sai_status_t sai_npu_lag_port_mode_get (sai_object_id_t lag_id,
                                               sai_object_id_t port_id, sai_lag_port_mode_t *mode)
{
    SAI_LAG_LOG_TRACE ("LAG port mode get, LAG ID: 0x%"PRIx64" Port ID: 0x%"PRIx64".",
                       lag_id, port_id);
...
static sai_npu_lag_api_t sai_vm_lag_api_table = {
    ...
    sai_npu_lag_port_mode_set,
    sai_npu_lag_port_mode_get,
