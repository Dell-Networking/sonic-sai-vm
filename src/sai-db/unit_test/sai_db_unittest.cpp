...
bool sai_route_test(){
    ...
    sai_attribute_t attrs[3];
    attrs[0].id = SAI_ROUTE_ATTR_NEXT_HOP_ID;
    attrs[0].value.oid = 1000;


    attrs[1].id = SAI_ROUTE_ATTR_PACKET_ACTION;
    attrs[1].value.u32 = 2;

    attrs[2].id = SAI_ROUTE_ATTR_TRAP_PRIORITY;
    attrs[2].value.u8 = 55;
    if ( sai_route_create_db_entry(&route_entry,3,attrs) != SAI_STATUS_SUCCESS) return false;

    sai_attribute_t set_attr;
    set_attr.id=SAI_ROUTE_ATTR_TRAP_PRIORITY;
